...
Unfortunately, all the previous measurements of O charge state distributions in gases employed atomic ions only \cite{Hubbard1955,preikschat1966charge,Liu2014}. The great majority of published experimental data on charge exchange cross sections are on neutral or single charged ions at relatively low energies ($E/M< 1$~keV/u) or on multiple charged ions at high energies ($E/M> 1$~MeV/u), and data from the intermediate energy regime (10 keV/u~$<E/M<1$~MeV/u) is more scarce. There are a few interesting points of comparison in the literature regarding the molecular effect on charge state distribution. Maor et al. \cite{maor1985charge} reported the charge distribution shifting towards lower charge states for molecular ions as compared to atomic ions. This was explained in terms of enhanced electron capture probability. Brunelle et al. \cite{brunelle1999reduced} measured the charge distributions of carbon clusters and compared them to carbon ions. One of the findings of Brunelle et al. \cite{brunelle1999reduced} suggest that the average charge state of carbon cluster constituents is much less than that of single carbon ions of the same velocity.  This was explained through the enhanced electron capture for closely spaced ions. 

Kumar et al. \cite{kumar2011charge} reported a difference in the stripping process for molecular beams as compared to atomic beams. Their study claims there can be some kind of interference due to the presence of the molecular ion passing through the stripping foil. By using two foils and using molecular beams they found the yield of the higher charge states could be increased. It is important to note that \cite{kumar2011charge} although reported higher yields of higher charge states, used two foil strippers in order to mitigate the vicinage effect which reduces high charge state yields. By using the first stripper they get low positive charge states and by subsequently passing the beam through a second stripper, they can access higher charge states.

% 7.  what will be done in this work, and why this is interesting?
...
An important aspect in  being able to detect any molecular effect is to quantify the uncertainties of the measured points. We have calculated the statistical uncertainty on all measured points in Figure \ref{fig:molecular_effect}  these can be seen in the form of error bars.  In order to get the maximum extent of the uncertainty we have fitted two additional curves  to our Gaussian fit of the experimental data points.  Therefore for each Gaussian fit of the experimental data point set at a particular terminal voltage, we have an upper fit and a lower fit. The upper fit consists of fitting the points of the first two upper limits of the error bars with the second two lower limits of the error bars. The lower fit consists of the opposite which means fitting the first two lower limits of the error bars with the second two upper limits of the error bars. For each charge state distribution we really only have four data points to work with (+5 is close to zero). By carrying out this procedure we are able to overestimate our uncertainty on the means and standard deviations of our experimental data sets. By taking the differences of the upper and lower fits we are able to determine the errors on the means and standard deviations of our experimental data Gaussian fits. Although this method overestimates the error, it strengthens the validity of our result.  The results can be seen in Table \ref{tab:gaussian_fitting_parameters}.  


\begin{table}[]
	\centering
	\caption{Gaussian fitting parameters of the four distributions studied}
	\begin{tabular}{lll}
		\hline
		fit  & mean            & standard deviation \\ \hline
		CasP & 2.422           & 0.825              \\
		\ce{BeO-} & 2.4884 $\pm$ 0.0007 & 0.8732 $\pm$ 0.0780    \\
		\ce{O-}  & 2.5920 $\pm$ 0.2127 & 0.8499 $\pm$ 0.0319    \\
		\ce{O2-} & 2.3006 $\pm$ 0.1020 & 0.7725 $\pm$ 0.0379    \\ \hline
	\end{tabular}
	\label{tab:gaussian_fitting_parameters}
\end{table}

Table \ref{tab:gaussian_fitting_parameters}, is presented pictorially in Figure \ref{fig:gaussian_fitting_comparison}. Here we can see that  \ce{BeO-} has the lowest uncertainty followed by \ce{O2-} and lastly \ce{O-} which has the greatest uncertainty. The means are closest between CasP and \ce{BeO-},  which is surprising because CasP calculates charge state distributions for atomic species  not molecular species, CasP should be closest to \ce{O-}. It is still possible that is still the case because the error bars for \ce{O-} are quite large, so that the means can fall within error range. One can also see there is a difference in means between \ce{O2-}; \ce{BeO-} and CasP; and beyond the error bars of \ce{O2-}. This indicates that there is a difference in charge state distribution even after overestimating our uncertainty. We would also like to point out that +2 of \ce{O2-} in Figure \ref{fig:molecular_effect} is far more pronounced and is the highest of all measured yields. Also comparing the width of these three distributions (c.f. Table \ref{table:rel_yield}), we can see that the width of the \ce{O2-} distribution is narrower at 0.77, whereas the width for \ce{BeO-} is 0.87 and the with for \ce{O-} is 0.85, which are quite similar.   We believe these are the contributing reasons why the \ce{O2-} charge state distribution is different to the rest. The large error bars in \ce{O-} make it difficult to say whether there is a molecular effect in that case. All we can say is there could be something. A possible explanation for what we are seeing could be that the gas pressure was set to give an equilibrium charge state distribution for \ce{BeO-} and the same pressure was used in all measurements.      

%Figure
\begin{figure}[h!] 
	\centering
	\includegraphics[width=0.75\textwidth]{gaussian_fitting_comparison}
	\caption{Graphical depiction of the Gaussian fitting parameters for easy comparison}
	\label{fig:gaussian_fitting_comparison}
	
\end{figure}

Now it is worth taking a step back and looking at our results in light of earlier literature. References \cite{maor1985charge, brunelle1999reduced, kumar2011charge} which are earlier alluded to concern molecular breakup in a solid stripper foil. Their findings suggest that molecular ions result in lower mean charge states due to increased electron capture as compared to atomic ions, although \cite{kumar2011charge} suggested it is possible to control this by using multiple strippers. In this sense our results agree with what is in literature as  \ce{O-} has the highest mean charge state followed by \ce{BeO-}  and finally  \ce{O2-}. Of course in our case as we have an argon stripper gas and not a solid carbon foil, in gaseous strippers we can assume the distance between stripper atoms is so huge and the interaction point distances are also huge that any electrons released during the interaction process (under equilibrium we have a balance between electron capture and electron loss) does not have an effect on the final charge state distribution. In stripper gasses we may also assume when injected molecules are broken the fragments are so far from each other that they don't interact with each other. However in the study of Fifield et al. \cite{fifield2023coulomb} reports that the Coulomb explosion is less for gas stripping as compared to sold strippers, but "it is still present". Indeed they do present results where \ce{^{9}Be^{3+}} of \ce{BeO-} origin experiences broadening of energy distribution when compared to similar ion of atomic origin in \ce{N2} stripping gas. We should also consider the uncertainties on  \ce{O-} and \ce{O2-} and possible differences between the calculation of CasP and experimental reality.     
